...
const TCP_STATES = {
  CLOSED: {
    ...
    desc: 'Connection is closed. Waiting to start 3-Way Handshake.',
  },
  SYN_SENT: {
    id: 'SYN SENT',
    color: '#f59e0b',
    bg: 'rgba(245, 158, 11, 0.1)',
    desc: 'Client sent SYN. Waiting for SYN-ACK.',
  },
  SYN_RECEIVED: {
    id: 'SYN RECEIVED',
    color: '#0ea5e9',
    bg: 'rgba(14, 165, 233, 0.1)',
    desc: 'Server sent SYN-ACK. Waiting for final ACK.',
  },
  ESTABLISHED: {
    id: 'ESTABLISHED',
    color: '#10b981',
  ...
  SLOW_START: {
    ...
    desc: 'CWND doubles every RTT. The sender aggressively probes the network for available bandwidth until the Slow Start Threshold (ssthresh) is reached.',
  },
  CONGESTION_AVOIDANCE: {
    id: 'CONGESTION AVOIDANCE',
    color: '#10b981',
    bg: 'rgba(16, 185, 129, 0.1)',
    desc: 'CWND grows linearly (+1 per RTT). The sender cautiously probes for more bandwidth to avoid causing a network traffic jam.',
  },
  FAST_RECOVERY: {
    id: 'FAST RECOVERY',
    color: '#f59e0b',
    bg: 'rgba(245, 158, 11, 0.1)',
    desc: 'Triggered by 3 Duplicate ACKs. ssthresh drops to CWND/2, and CWND drops to ssthresh. Skipped the harsh Slow Start phase for a faster recovery.',
  },
  TIMEOUT: {
    id: 'TIMEOUT',
    color: '#ef4444',
    bg: 'rgba(239, 68, 68, 0.1)',
    desc: 'Severe congestion detected (ACKs not returning). ssthresh drops to CWND/2, and CWND resets entirely back to 1. Back to Slow Start.',
  },
};
...
function initTCPVisualizer() {
  ...
  els.btnStepHandshake.addEventListener('click', stepHandshake);

  els.btnForceDupAck.addEventListener('click', () => {
    if (simState.isRunning) simState.forceDupAck = true;
  });
  els.btnForceTimeout.addEventListener('click', () => {
    if (simState.isRunning) simState.forceTimeout = true;
  });
...
function toggleSimulation() {
  ...
  if (simState.state === TCP_STATES.ESTABLISHED) {
    simState.state = TCP_STATES.SLOW_START; // start congestion control
  }
...
function stepHandshake() {
  const w = els.canvasContainer.getBoundingClientRect().width;
  const speed = 1.0 / (60 * (simState.rttMs / 1000) * 0.8);

  if (simState.state === TCP_STATES.CLOSED) {
    simState.state = TCP_STATES.SYN_SENT;
    els.handshakeNextStep.textContent = 'Send SYN-ACK';
    packets.push({
      id: Math.random(),
      x: 100,
      y: 150,
  ...
  } else if (simState.state === TCP_STATES.SYN_SENT) {
    simState.state = TCP_STATES.SYN_RECEIVED;
    els.handshakeNextStep.textContent = 'Send ACK';
    packets.push({
      id: Math.random(),
      x: w - 100,
      y: 150,
  ...
  } else if (simState.state === TCP_STATES.SYN_RECEIVED) {
    simState.state = TCP_STATES.ESTABLISHED;
    els.handshakeNextStep.textContent = 'Connection Established';
    els.btnStepHandshake.disabled = true;
    packets.push({
      id: Math.random(),
      x: 100,
      y: 150,
...
function renderCanvas() {
  ...
  for (let i = packets.length - 1; i >= 0; i--) {
    ...
    } else if (p.type === 'SYN' || p.type === 'ACK') {
      if (p.progress <= 1.0) {
        p.x = senderX + (receiverX - senderX) * p.progress;
        drawPacket(p.x, p.y, '#f59e0b', false, p.type);
      } else {
        packets.splice(i, 1);
      }
    } else if (p.type === 'SYN_ACK') {
      if (p.progress <= 2.0) {
        p.x = receiverX - (receiverX - senderX) * (p.progress - 1.0);
        drawPacket(p.x, p.y, '#0ea5e9', false, p.type);
      } else {
        packets.splice(i, 1);
      }
...
function drawPacket(x, y, color, isAck = false, label = null) {
  ctx.beginPath();
  if (isAck && !label) {
    // Small circle for standard ACK in data phase
    ctx.arc(x, y, 5, 0, Math.PI * 2);
  } else {
    // Rectangle for Data or Labeled Packets (SYN/SYN-ACK)
    const w = label ? 40 : 16;
