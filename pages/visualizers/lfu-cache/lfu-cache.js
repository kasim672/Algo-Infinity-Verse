...
class LFUVisualizer {
  ...
  createDOMNode(node) {
    ...
    el.innerHTML = `
            ...
            <span class="node-t">t=${node.timestamp}</span>
        `;
  ...
  updateTelemetry() {
    const telHits = document.getElementById('tel-hits');
    const telMisses = document.getElementById('tel-misses');
    const telSize = document.getElementById('tel-size');
    const telMinFreq = document.getElementById('tel-minfreq');

    if (telHits) telHits.innerText = this.hits;
    if (telMisses) telMisses.innerText = this.misses;
    if (telSize) telSize.innerText = `${this.size} / ${this.capacity}`;
    if (telMinFreq) telMinFreq.innerText = this.minFreq;
  }
  ...
  *getRoutine(k) {
    ...
    node.timestamp = ++this.tick;
    const domEl = document.getElementById(node.domId);
    if (domEl) {
      domEl.querySelector('.node-t').innerText = `t=${node.timestamp}`;
    }

    this.updateMath(
      `Node <span class="eq-hl">[${k}]</span> found. <br> Current Freq = <span class="eq-p">${oldFreq}</span>, Updated t=${node.timestamp}`
    );
    yield;
    ...
    if (this.freqMap.get(oldFreq).size === 0 && this.minFreq === oldFreq) {
      ...
      this.renderTierBackgrounds();
      this.updateMath(
        `freqMap[${oldFreq}] is empty and minFreq == ${oldFreq}. <br> <span class="eq-w">minFreq incremented to ${this.minFreq}</span>`
      );
      yield;
    }
  ...
  *getAlgo(k, _isStandalone) {
    ...
    if (!this.keyMap.has(k)) {
      ...
      this.updateTelemetry();
      this.updateMath(
        `Key <span class="eq-hl">[${k}]</span> not in Map. <br><span class="eq-err">Cache Miss.</span>`
      );
      yield;
  ...
  *putAlgo(k, v) {
    ...
    if (this.keyMap.has(k)) {
      ...
      this.updateMath(
        `Key <span class="eq-hl">[${k}]</span> exists. Updated value to <span class="eq-hl">${v}</span>`
      );
      this.highlightMapRow(k);
      yield;
    ...
    if (this.size >= this.capacity) {
      this.updateMath(
        `Cache Full (${this.size}/${this.capacity}). <br><span class="eq-err">Must Evict LRU from minFreq tier.</span>`
      );
      yield;
      ...
      this.updateMath(
        `Evicting Tail (LRU tie-breaker): Key <span class="eq-err">[${evictNode.key}]</span> (t=${evictNode.timestamp})`
      );
      yield;
    ...
    const newNode = new LFUNode(k, v, ++this.tick);
    this.createDOMNode(newNode);
    this.setNodeActive(newNode, true);
    this.updateMath(`Created new Node [${k}:${v}] at t=${newNode.timestamp}. Initial Freq = 1.`);
    yield;
  ...
  // --- Control Flow ---
  startAlgorithm(generator, _mode) {
